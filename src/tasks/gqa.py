...
class GQA:
    ...
    def topk_predict(self, eval_tuple: DataTuple, dump=None):
        # if target in topk predictions, then it is correct
        self.model.eval()
        dset, loader, evaluator = eval_tuple
        quesid2ans = {}
        for i, datum_tuple in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                logit = self.model(feats, boxes, sent)
                topk = logit.topk(k=4, dim=1)
                for qid, preds, targ in zip(ques_id, topk.indices.cpu().numpy(), target.numpy()):
                    proposals = [dset.label2ans[l] for l in preds]
                    ans = dset.label2ans[targ]
                    if ans in proposals:
                        quesid2ans[qid] = ans
                    else:
                        quesid2ans[qid] = proposals[0]
        if dump is not None:
            evaluator.dump_result(quesid2ans, dump)

...
from tasks.gqa_model_vis import GQAModel
...
class GQA:
    def __init__(self):
        ...
        if args.backbone == "lxmert":
            ...
            self.model = GQAViLT(self.train_tuple.dataset.num_answers - 1)
        ...
        for epoch in range(args.epochs):
            ...
            for i, (ques_id, feats, boxes, sent, target) in iter_wrapper(enumerate(loader)):
                ...
                if args.backbone in ['lxmert', 'butd', 'uniter']:
                    ...
                    logit = self.model(feats, boxes, sent, output_attention=True)
    ...
    def ood_evaluate(self, eval_tuple: DataTuple, dump=None):
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            with torch.no_grad():
                if args.backbone in ['lxmert', 'butd', 'uniter']:
                    feats, boxes = feats.cuda(), boxes.cuda()
                    _, att_probs, input_ids = self.model(feats, boxes, sent, output_attention=True)
                    l2v_att = att_probs[0][-1].mean(1)
                    v2l_att = att_probs[0][-1].mean(1)
                elif args.backbone in ['vilt']:
                    feats = feats.cuda()
                    encoded = self.tokenizer(
                        sent,
                    ...
                    logit = self.model(batch)
                # score, label = torch.sigmoid(logit).max(1)
                for qid, l2v, v2l, iid in zip(ques_id, l2v_att.cpu().numpy(), v2l_att.cpu().numpy(), input_ids.cpu().numpy()):
                    # ans = dset.label2ans[l]
                    # quesid2ans[qid] = (ans, s)
                    quesid2att[qid] = {'l2v':l2v, 'v2l':v2l}
                    quesid2att[qid]['img_id'] = dset.id2datum[qid]['img_id']
                    img_info = loader.dataset.imgid2img[quesid2att[qid]['img_id']]
                    quesid2att[qid]['box'] = img_info['boxes'].copy()
                    end_idx = np.where(iid == 102)[0][0]
                    quesid2att[qid]['input_id'] = iid[1:end_idx]
        # results = evaluator.evaluate(quesid2ans)
        # if dump is not None:
            # evaluator.dump_result(quesid2ans, dump)
        # return results
        with open("atts/atts.pickle", 'wb') as f:
            pickle.dump(quesid2att, f, protocol=pickle.HIGHEST_PROTOCOL)

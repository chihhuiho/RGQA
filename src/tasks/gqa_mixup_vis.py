...
class GQA:
    def __init__(self):
        ...
        if args.backbone in ['lxmert', 'uniter', 'butd']:
            ...
            for i, (ques_id, feats, boxes, sent, target) in iter_wrapper(enumerate(loader)):
                ...
                if args.mixup_mode == 'perturb':
                    ...
                    for j, qid in enumerate(ques_id):
                        ...
                        # feats: (B, N, size)
                        prop = np.random.beta(args.mixup_alpha, args.mixup_beta) #random.random()


                        object_num = feat_pos.shape[0]
                        random_idx = np.arange(object_num)
                        np.random.shuffle(random_idx)
                        random_idx = random_idx[:int(prop * object_num)]
                       

                        if args.mixup_mode == 'mixup_v3':
                            pet_feat = torch.zeros_like(feat_neg) 
                        else:
                            pet_feat = feat_neg.clone().detach()
                        pet_feat[random_idx, :] = feat_pos[random_idx, :]
                    
                        box_pos = boxes[j]
                        box_neg = boxes[ques_id.index(qid_r)]
 
                        pet_box = box_neg.clone().detach()
                        pet_box[random_idx, :] = box_pos[random_idx, :]
                        
                        pet_feats.append(pet_feat)
                        pet_boxes.append(pet_box)
                        if args.mixup_mode == 'mixup_v1' or args.mixup_mode == 'mixup_v3':

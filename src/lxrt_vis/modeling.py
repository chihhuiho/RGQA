...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None, output_attention=False):
...
class BertCrossattLayer(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None, output_attention=False):
        output, att = self.att(input_tensor, ctx_tensor, ctx_att_mask, output_attention)
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        self_output, _ = self.self(input_tensor, input_tensor, attention_mask)
...
class LXRTXLayer(nn.Module):
    ...
    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask, output_attention=False):
        # Cross Attention
        lang_att_output,l2v_att = self.visual_attention(lang_input, visn_input, ctx_att_mask=visn_attention_mask, output_attention=output_attention)
        visn_att_output, v2l_att = self.visual_attention(visn_input, lang_input, ctx_att_mask=lang_attention_mask, output_attention=output_attention)
        return lang_att_output, visn_att_output, l2v_att, v2l_att
    ...
    def forward(self, lang_feats, lang_attention_mask,
                      visn_feats, visn_attention_mask, output_attention=False):
        lang_att_output = lang_feats
        visn_att_output = visn_feats

        lang_att_output, visn_att_output, l2v_att, v2l_att = self.cross_att(lang_att_output, lang_attention_mask,
                                                          visn_att_output, visn_attention_mask, output_attention=output_attention)
        ...
        return lang_output, visn_output, l2v_att, v2l_att
...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
                visn_feats, visn_attention_mask=None, output_attention=False):
        ...
        for layer_module in self.x_layers:
            lang_feats, visn_feats, l2v, v2l = layer_module(lang_feats, lang_attention_mask,
                                                  visn_feats, visn_attention_mask,
                                                  output_attention=output_attention)
            l2v_atts.append(l2v)
            v2l_atts.append(v2l)

        return lang_feats, visn_feats, l2v_atts, v2l_atts
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None, output_attention=False):
        ...
        # Run LXRT backbone
        lang_feats, visn_feats, l2v_atts, v2l_atts = self.encoder(
            embedding_output,
        ...
        return (lang_feats, visn_feats), pooled_output, (l2v_atts, v2l_atts)
...
class LXRTPretraining(BertPreTrainedModel):
    ...
    def forward_match(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, pos=None):
        """
        output binary score indicating if the image and text match
        """
        (lang_output, _), pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask,
            visual_feats=(visual_feats, pos),
        )

        _, cross_relationship_score = self.cls(lang_output, pooled_output)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None, output_attention=False):
        feat_seq, pooled_output, att_probs = self.bert(input_ids, token_type_ids, attention_mask,
                                            visual_feats=visual_feats,
                                            visual_attention_mask=visual_attention_mask,
                                            output_attention=output_attention)
        if 'x' == self.mode:
            return pooled_output, att_probs
        elif 'x' in self.mode and ('l' in self.mode or 'r' in self.mode):
            return feat_seq, pooled_output, att_probs
        elif 'l' in self.mode or 'r' in self.mode:
            return feat_seq, att_probs
